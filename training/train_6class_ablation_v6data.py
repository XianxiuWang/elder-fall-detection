#!/usr/bin/env python3
"""
train_6class_ablation_v6data.py — 消融: v6 数据源 + v8 协议(GroupKFold 5折 + 事件级)
====================================================================
严格同口径对比实验:
  - 数据源 = v6 的 6 个源 (无 custom_standup_wakeup / custom_sitdown)
  - 不去重 (保留 custom_6class/5class 里旧的 category 2/4 文件)
  - 评估协议 = 与 v8 完全一致 (GroupKFold 5折 + 事件级评估 + AP/ROC-AUC)

用途: 回答 "v6 的数据在公平协议下到底是什么水平", 与 v8 直接对照。
"""
...
# 消融: 不去重, 保留旧 category 2/4 文件 (复现 v6 数据)
DEDUP_SOURCES = ()

MODEL_OUT = r"E:\老人跌倒\models\fall_classifier_6class_v6data_5fold.pkl"
...
def main():
    print("=" * 60)
    print("  Six-Class Classifier [ABLATION v6-data] — GroupKFold 5-fold, v6 数据源无去重")
    print("=" * 60)
    ...
    bundle = {
        ...
        "config": {
            ...
            "n_folds": N_FOLDS, "version": "v6data-5fold",
        },
        "metrics": {
    ...
    print(f"\n{'='*60}")
    print(f"  ABLATION (v6-data, 5-fold) TRAINING COMPLETE!")
